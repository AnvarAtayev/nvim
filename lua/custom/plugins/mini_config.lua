...
return {
    ...
    config = function()
        ...
        vim.api.nvim_set_hl(0, 'CustomModifiedStatus', {
            fg = '#FFCC66',
            bold = true
        })
        ...
        vim.api.nvim_create_autocmd({'BufModifiedSet', 'BufEnter', 'ColorScheme'}, {
            group = vim.api.nvim_create_augroup('CustomModifiedStatusGroup', {
                clear = true
            }),
            callback = function(args)
                if vim.bo[args.buf].modified then
                    ...
                    vim.api.nvim_set_hl(0, 'MiniStatuslineFilename', {
                        link = 'CustomModifiedStatus'
                    })
                else
                    -- Link it back to the default group when the buffer is clean
                    vim.api.nvim_set_hl(0, 'MiniStatuslineFilename', {
                        link = 'MiniStatuslineFilenameNormal'
                    })
                end
